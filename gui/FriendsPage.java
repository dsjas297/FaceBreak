...
public class FriendsPage extends JPanel{
	...
	**/
	public FriendsPage(MouseListener parent, FBClient myClient, int wall_width, ArrayList<String> friendsList){
		...
		if (friendsList == null || friendsList.size() == 0){
			...
			for (int i=0; i<friendsList.size(); i++){
				try {
					//get profile
					Profile myProfile = new Profile(friendsList.get(i));
					myClient.viewProfile(myProfile);
					
					//put information into a box
					Userlink name = new Userlink(myProfile.getFname() + " " + myProfile.getLname(), friendsList.get(i));
