...
//		if(outStream != null) outStream.close();
	...
	private boolean establishSecureConnection() throws Exception {
		...
		try {
			...
			KeyPairGenerator kpg = KeyPairGenerator.getInstance(DHKey.PROTOCOL);
...
//			return Error.MALFORMED_REQUEST;
		...
		ContentSingle<FBClientUser> userInfo = new ContentSingle<FBClientUser>();
	...
	public Error viewProfile(Profile profile) throws ClassNotFoundException {
		...
		ContentSingle<Profile> profileReq = new ContentSingle<Profile>();
		...
		if (e == Error.SUCCESS) {
			Profile tmp = ((ContentSingle<Profile>) serverReply.getDetails()).get();
	...
	public Error post(Post newPost) throws ClassNotFoundException {
		...
		ContentSingle<Post> postInfo = new ContentSingle<Post>();
	...
	public Error viewRegion(Region region) throws ClassNotFoundException {
		...
		ContentSingle<Region> regionInfo = new ContentSingle<Region>();
		...
		if(e == Error.SUCCESS) {
			ContentList<Post> tmp = (ContentList<Post>)serverReply.getDetails();
			if(tmp.getSize() > 0) {
...
//		delete.getDetails().setPost(new Post());
	...
	public Error addFriend(String friendName) throws ClassNotFoundException {
		...
		ContentSingle<String> friendInfo = new ContentSingle<String>();
	...
	public Error deleteFriend(String exFriendName) throws ClassNotFoundException {
		...
		ContentSingle<String> exFriendInfo = new ContentSingle<String>();
	...
	public Error getFriendsList() throws ClassNotFoundException {
		// sanity check
		if(socket == null || user == null)
			return Error.LOGIN;
		
		Request getFriendsList = new Request(RequestType.LIST_FRIENDS);
